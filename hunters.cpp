...
void hunter_spawn(hunter_t *hunter, player_t *player) {
        ...
        for (int i = 0; i < MAX_AMM_HUNTERS; i++) {
            ...
            if (rand() % 100 <= hunter[i].spawn_chance && player->current_amm_of_hunters_on_board <= MAX_AMM_HUNTERS) {
            if (hunter[i].is_active == FALSE) {
                ...
                hunter[i].vel.x = u_x * 0.2f * EVALUATION_BY_LEVEL * EVALUATION_BY_TIME; // speed evaluated by time and level
                hunter[i].vel.y = u_y * 0.2f * EVALUATION_BY_LEVEL * EVALUATION_BY_TIME;
                // -----------------
